...
def examples_to_trials(examples, params):  #52 (line in Coconut source)
    ...
    for tid, ex in enumerate(examples):  #57 (line in Coconut source)
        ...
        result = {"status": STATUS_OK, "loss": loss}  #63 (line in Coconut source)

        vals = {}  #68 (line in Coconut source)
        idxs = {}  #69 (line in Coconut source)
        for k, v in get_names_and_features(ex["values"], params, fallback_func=lambda name, func, *args, **kwargs: NA, converters={"choice": lambda val, choices: choices.index(val), "randrange": lambda val, start, stop, step: val - start}, convert_fallback=False):  #70 (line in Coconut source)
            vals[k] = [v,] if v is not NA else []  #80 (line in Coconut source)
            idxs[k] = [tid,] if v is not NA else []  #81 (line in Coconut source)

        misc = {"tid": tid, "idxs": idxs, "vals": vals, "cmd": None}  #83 (line in Coconut source)

        trials.append({"tid": tid, "result": result, "misc": misc, "spec": spec_from_misc(misc), "state": JOB_STATE_DONE, "owner": None, "book_time": None, "refresh_time": None, "exp_key": None})  #90 (line in Coconut source)
...
class HyperoptBackend(StandardBackend):  #107 (line in Coconut source)
    ...
    def setup_backend(self, params, algo=tpe.suggest, rstate=None, show_progressbar=False, **options):  #119 (line in Coconut source)
        ...
        space = (as_apply)(dict(((name), (create_space(name, func, *args))) for name, (func, args, kwargs) in sorted_items(params)))  #128 (line in Coconut source)
...
#  to the self.set_current_values callback passed to Domain
    ...
    def set_current_values(self, values):  #160 (line in Coconut source)
        ...
        return {"status": STATUS_RUNNING}  #164 (line in Coconut source)
