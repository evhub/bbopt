...
def call_test(args, ignore_errs=(), prepend_py=True):  #100 (line in Coconut source)
    ...
    for line in stderr.splitlines():  #109 (line in Coconut source)
        if not any((ignore in line for ignore in _coconut.itertools.chain.from_iterable(_coconut_reiterable(_coconut_func() for _coconut_func in (lambda: always_ignore_errs, lambda: ignore_errs))))):  #110 (line in Coconut source)
...
class TestExamples(unittest.TestCase):  #170 (line in Coconut source)
    ...
    def test_mixture(self):  #306 (line in Coconut source)
        ...
        with using(mixture_data):  #308 (line in Coconut source)
            ...
            assert (len)((set)((map)(_coconut_base_compose(_coconut.operator.itemgetter(("memo")), (_coconut.operator.itemgetter(("alg")), 0)), mixture_example.bb.get_data()["examples"]))) > 1  #320 (line in Coconut source)
